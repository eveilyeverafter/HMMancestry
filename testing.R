...
l=200; scale = 0.01052632; snps=2*(1:l);
n.tetrads <- 100 # number of spores to simulate
res <- sim_tetrad(n.tetrads=n.tetrads, l=l, scale=scale, snps=snps, p.assign=0.9210505, 
   mu.rate=0, f.cross=0.5, f.convert=0.9, length.conversion=10, coverage=10)


# Convert to proper dataframe:
dat <- lapply(1:100, function(x){
...
pars <-   expand.grid(p.assign=seq(from=0.9, to=0.99999, length.out=20), 
                      scale=seq(from=0, to=.2, length.out=20))
